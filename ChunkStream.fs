...
let parseMessage0 (msg_bytes: byte array, total: int) = 
    if total <= 1 then Error "Not enough bytes for a parsed M0"
    else 
        createVersion msg_bytes[0] 
        |> Result.bind (fun ver -> Ok {version = ver})

let receive (state: AppState) =
    match state.handshake_state with
    | Uninitialized ->
        let msg_raw = recv state.socket
        parseMessage0 msg_raw
        |> Result.bind (receiveM0 state.socket)
        |> Result.bind (fun _ -> Ok VersionSent)
